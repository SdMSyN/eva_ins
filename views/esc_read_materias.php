...
<?php
    ...
    if (!isset($_SESSION['sessU'])){
        ...
        //Obtenemos los grados del nivel escolar de la escuela
        $sqlGetGrados = "SELECT id, nombre FROM $tGrado WHERE nivel_escolar_id='$idNivelEsc' ";
        $resGetGrados = $con->query($sqlGetGrados);
        $optGrado = '<option></option>';
        while($rowGetGrados = $resGetGrados->fetch_assoc()){
            $optGrado .= '<option value="'.$rowGetGrados['id'].'">'.$rowGetGrados['nombre'].'</option>';
        }
            ...
            <table class="table table-striped" id="data">
                <thead>
                    <tr>
                        ...
                        <th><span title="creador">Creador</span></th>
                        <th>Ver grupo</th>
                    </tr>
                </thead>
                <tbody></tbody>
                    ...
                    <form id="formAdd" name="formAdd">
                        <div class="modal-body">
                            <div class="form-group">
                                <input type="hidden" name="inputNivEsc" value="<?=$idNivelEsc;?>" >
                                <input type="hidden" name="inputIdUser" value="<?=$idUser;?>" >
                                <label for="inputGrado">Grado: </label>
                                <select class="form-control" id="inputGrado" name="inputGrado" ><?=$optGrado;?></select>
                            </div>
                            <div class="form-group">
        ...
        $(document).ready(function(){
           ...
           function filtrar(){
               $.ajax({
                   type: "POST",
                   data: ordenar, 
                   url: "../controllers/esc_get_materias.php?idEsc="+<?=$idUser;?>+"&idNivelEsc="+<?=$idNivelEsc;?>,
                       ...
                       if(msg.error == 0){
                           ...
                           $.each(msg.dataRes, function(i, item){
                               var newRow = '<tr>'
                                    ...
                                    newRow += (msg.dataRes[i].creador == null) ? '<td>Plataforma</td>' : '<td>Escuela</td>' 
                                    newRow += '</tr>';
           ...
           $('#formAdd').validate({
                ...
                messages: {
                    inputGrado: "Grado de la materia obligatorio",
                ...
                submitHandler: function(form){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../controllers/esc_create_banco_materia.php",
                        ...
                        success: function(msg){
                            ...
                            if(msg.error == 0){
                                ...
                                setTimeout(function () {
                                  location.href = 'esc_read_materias.php';
                                }, 1500);
                            }else{
