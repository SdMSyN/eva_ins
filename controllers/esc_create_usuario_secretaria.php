<?php
    ...
    $sqlGetNumSecs = "SELECT id FROM $tSec ";
    $resGetNumSecs = $con->query($sqlGetNumSecs);
    $getNumSecs = $resGetNumSecs->num_rows;
    //Creamos clave usuario y contraseña
    $nombre = $ap.' '.$am.' '.$name;
    $apTmp = str_replace(' ', '', $ap);
    $clave = strtolower($name{0}).strtolower($apTmp).strtolower($am{0}).$getNumSecs;
    ...
    if($con->query($sqlInsertInfo) === TRUE){
        ...
        $sqlInsertUser = "INSERT INTO $tSec "
        ...
        if($con->query($sqlInsertUser) === TRUE){
            ...
            $ban = false;
            $cadErr .= 'Error al crear nueva secretaria<br>'.$con->error;
        }
    }else{
        $ban = false;
