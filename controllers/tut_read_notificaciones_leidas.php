<?php
    ...
    $sqlGetAvInfo = "SELECT $tAvAsigT.id as id, $tAvAsigT.enterado as enterado, "
            . "$tAvInfo.nombre, $tAvTipo.nombre as tipo, $tAvInfo.dirigido_a, "
            . "$tAvInfo.perfil_creador, $tAvInfo.creado_por, $tAvInfo.id as idAvInfo, $tAvInfo.creado "
            . "FROM $tAvAsigT "
            . "INNER JOIN $tAvInfo ON $tAvInfo.id=$tAvAsigT.aviso_info_id "
            . "INNER JOIN $tAvTipo ON $tAvTipo.id=$tAvInfo.tipo_aviso_id "
            . "WHERE $tAvAsigT.tutor_id='$idTut' AND $tAvAsigT.enterado IS NOT NULL "
            . "ORDER BY creado ";
    //Ordenar ASC y DESC
